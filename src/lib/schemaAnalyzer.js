...
class SchemaAnalyzer {
  ...
  async getCurrentSchemas() {
    ...
    for (const file of modelFiles) {
      try {
        ...
        // Extract schema definition using more flexible regex
        const schemaMatch = content.match(/new\s+Schema\s*<[^>]*>\s*\(\s*{([\s\S]*?)}\s*,?\s*{([\s\S]*?)}?\s*\)/);
        
        if (schemaMatch) {
          const modelName = file.replace('.ts', '');
          console.log(`✅ Found schema for ${modelName}`);
          
          schemas[modelName] = {
            fields: this.parseSchemaFields(schemaMatch[1]),
            options: this.parseSchemaOptions(schemaMatch[2] || '{}'),
            indexes: this.extractIndexes(content)
          };
        } else {
          console.log(`⚠️  No schema found in ${file}`);
        }
      } catch (error) {
  ...
  /**
   * Parse schema fields from string
   */
  parseSchemaFields(fieldsString) {
    const fields = {};
    
    // Split by field definitions (look for fieldName: { ... })
    ...
    for (const match of fieldMatches) {
      ...
      // Skip if it's a nested object (like address: { ... })
   ...
   */
  parseSchemaOptions(optionsString) {
    const options = {};
    
    // Extract timestamps
    const timestampsMatch = optionsString.match(/timestamps:\s*(true|false)/);
    if (timestampsMatch) {
  ...
  /**
   * Extract indexes from schema content
   */
