...
export async function scrapeBeOneShopOne(options: { limit?: number } = {}): Promise<ScrapeResult> {
    const baseUrl = 'https://beoneshopone.com/products.json';
    const products: MappedProduct[] = [];
    let page = 1;
    let hasMore = true;
    const limit = 250;

    console.log(`[BeOneShopOne] Starting scrape...`);

    try {
        while (hasMore) {
            console.log(`[BeOneShopOne] Fetching page ${page}...`);
            ...
            for (const product of pageProducts) {
                ...
                const images = product.images?.map(img => cleanImageUrl(img.src, 'https://beoneshopone.com')) || [];
                if (images.length === 0 && product.image) {
                    images.push(cleanImageUrl(product.image.src, 'https://beoneshopone.com'));
                }
                ...
                const mappedProduct: MappedProduct = {
                    ...
                    category: category,
                    brand: 'beoneshopone',
                    sourceUrl: `https://beoneshopone.com/products/${product.handle}`,
        ...
        console.error(`[BeOneShopOne] Error: ${error.message}`);
    }
