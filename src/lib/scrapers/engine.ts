...
export async function scrapeEngine(options: { limit?: number } = {}): Promise<ScrapeResult> {
    const baseUrl = 'https://engine.com.pk/products.json';
    const products: MappedProduct[] = [];
    let page = 1;
    let hasMore = true;
    const limit = 250;

    console.log(`[Engine] Starting scrape...`);

    try {
        while (hasMore) {
            console.log(`[Engine] Fetching page ${page}...`);
            ...
            for (const product of pageProducts) {
                ...
                const images = product.images?.map(img => cleanImageUrl(img.src, 'https://engine.com.pk')) || [];
                if (images.length === 0 && product.image) {
                    images.push(cleanImageUrl(product.image.src, 'https://engine.com.pk'));
                }
                ...
                const mappedProduct: MappedProduct = {
                    ...
                    category: category,
                    brand: 'engine',
                    sourceUrl: `https://engine.com.pk/products/${product.handle}`,
