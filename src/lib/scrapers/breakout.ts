...
export async function scrapeBreakout(options: { limit?: number } = {}): Promise<ScrapeResult> {
    const baseUrl = 'https://breakout.com.pk/products.json';
    const products: MappedProduct[] = [];
    let page = 1;
    let hasMore = true;
    const limit = 250;

    console.log(`[Breakout] Starting scrape...`);

    try {
        while (hasMore) {
            console.log(`[Breakout] Fetching page ${page}...`);
            ...
            for (const product of pageProducts) {
                ...
                const images = product.images?.map(img => cleanImageUrl(img.src, 'https://breakout.com.pk')) || [];
                if (images.length === 0 && product.image) {
                    images.push(cleanImageUrl(product.image.src, 'https://breakout.com.pk'));
                }
                ...
                const mappedProduct: MappedProduct = {
                    ...
                    category: category,
                    brand: 'breakout',
                    sourceUrl: `https://breakout.com.pk/products/${product.handle}`,
