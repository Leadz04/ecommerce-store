...
export async function scrapeAlmas(options: { limit?: number } = {}): Promise<ScrapeResult> {
    const baseUrl = 'https://www.almas.pk/products.json';
    const products: MappedProduct[] = [];
    let page = 1;
    let hasMore = true;
    const limit = 250;

    console.log(`[Almas] Starting scrape...`);

    try {
        while (hasMore) {
            console.log(`[Almas] Fetching page ${page}...`);
            ...
            for (const product of pageProducts) {
                ...
                const images = product.images?.map(img => cleanImageUrl(img.src, 'https://www.almas.pk')) || [];
                if (images.length === 0 && product.image) {
                    images.push(cleanImageUrl(product.image.src, 'https://www.almas.pk'));
                }
                ...
                const mappedProduct: MappedProduct = {
                    ...
                    category: category,
                    brand: 'almas',
                    sourceUrl: `https://www.almas.pk/products/${product.handle}`,
        ...
        console.error(`[Almas] Error: ${error.message}`);
    }
