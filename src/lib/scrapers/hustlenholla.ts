...
export async function scrapeHustleNHolla(options: { limit?: number } = {}): Promise<ScrapeResult> {
    const baseUrl = 'https://hustlenholla.com.pk/products.json';
    const products: MappedProduct[] = [];
    let page = 1;
    let hasMore = true;
    const limit = 250;

    console.log(`[HustleNHolla] Starting scrape...`);

    try {
        while (hasMore) {
            console.log(`[HustleNHolla] Fetching page ${page}...`);
            ...
            for (const product of pageProducts) {
                ...
                const images = product.images?.map(img => cleanImageUrl(img.src, 'https://hustlenholla.com.pk')) || [];
                if (images.length === 0 && product.image) {
                    images.push(cleanImageUrl(product.image.src, 'https://hustlenholla.com.pk'));
                }
                ...
                const mappedProduct: MappedProduct = {
                    ...
                    category: category,
                    brand: 'hustlenholla',
                    sourceUrl: `https://hustlenholla.com.pk/products/${product.handle}`,
        ...
        console.error(`[HustleNHolla] Error: ${error.message}`);
    }
