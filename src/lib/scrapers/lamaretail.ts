...
export async function scrapeLamaRetail(options: { limit?: number } = {}): Promise<ScrapeResult> {
    const baseUrl = 'https://pk.lamaretail.com/products.json';
    const products: MappedProduct[] = [];
    let page = 1;
    let hasMore = true;
    const limit = 250;

    console.log(`[LamaRetail] Starting scrape...`);

    try {
        while (hasMore) {
            console.log(`[LamaRetail] Fetching page ${page}...`);
            ...
            for (const product of pageProducts) {
                ...
                const images = product.images?.map(img => cleanImageUrl(img.src, 'https://pk.lamaretail.com')) || [];
                if (images.length === 0 && product.image) {
                    images.push(cleanImageUrl(product.image.src, 'https://pk.lamaretail.com'));
                }
                ...
                const mappedProduct: MappedProduct = {
                    ...
                    category: category,
                    brand: 'lamaretail',
                    sourceUrl: `https://pk.lamaretail.com/products/${product.handle}`,
        ...
        console.error(`[LamaRetail] Error: ${error.message}`);
    }
