...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Order saved successfully (pending payment):', {
      _id: order._id,
      orderNumber: order.orderNumber,
      userId: order.userId,
      total: order.total,
    ...
    // NOTE:
    // Do NOT send any "new order" emails here.
    // At this point the payment has not been confirmed yet (paymentStatus is usually 'pending'),
    // so all customer/admin email notifications should be triggered only AFTER Stripe confirms
    // successful payment (handled in `/api/payments/webhook` and frontend `handlePaymentSuccess`).
