...
interface NineNineNineResponse {
  products: NineNineNineProduct[];
}
...
// Helper function to convert price (Shopify prices are in cents, but 999.com.pk seems to use decimal)
...
async function fetchAllProductsWithPagination(baseUrl: string): Promise<NineNineNineProduct[]> {
  const allProducts: NineNineNineProduct[] = [];
  let page = 1;
  ...
  while (hasMore) {
    try {
      ...
      console.error(`[999.com.pk Scraper] Error fetching page ${page}:`, error.message);
