...
function extractTags(description: string, title: string, productType?: string, tags?: string[]): string[] {
  ...
  if (text.includes('shoe')) extractedTags.push('shoe');
...
function convertShopifyToProduct(shopifyProduct: ShopifyProduct): any {
  ...
  const images = shopifyProduct.images?.map(img => {
    ...
    if (img.src.startsWith('/')) {
      return `https://unze.com.pk${img.src}`;
    }
    return img.src;
  }) || [];
  ...
  return {
    ...
    images: images,
    category: category,
    brand: 'unze', // Set brand to "unze" as requested
    sourceUrl: sourceUrl,
...
async function fetchUnzeProducts(page = 1, limit = 250): Promise<{ products: ShopifyProduct[]; hasNext: boolean }> {
  try {
    const url = `https://unze.com.pk/products.json?page=${page}&limit=${limit}`;
    console.log(`[Unze Scraper] Fetching page ${page} from ${url}`);
    ...
    console.error(`[Unze Scraper] Error fetching page ${page}:`, error.message);
    throw error;
  }
}
  ...
  try {
    ...
    while (hasNext) {
      try {
        ...
        console.error(`[Unze Scraper] Error on page ${page}:`, error.message);
    ...
    const allProductsJsonData = {
      ...
      source: 'https://unze.com.pk/products.json',
      brand: 'unze',
