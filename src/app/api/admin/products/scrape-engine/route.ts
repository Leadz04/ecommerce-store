...
function convertShopifyToProduct(shopifyProduct: ShopifyProduct): any {
  ...
  let images = shopifyProduct.images?.map(img => {
    ...
    if (img.src.startsWith('/')) {
      return `https://engine.com.pk${img.src}`;
    }
    return img.src;
  }) || [];
  ...
  if (images.length === 0 && shopifyProduct.image) {
    ...
    if (imgSrc.startsWith('//')) {
      ...
      cleanSrc = `https://engine.com.pk${imgSrc}`;
    }
    images = [cleanSrc];
  }
  ...
  const sourceUrl = `https://engine.com.pk/products/${shopifyProduct.handle}`;
  ...
  return {
    ...
    images: images,
    category: category,
    brand: 'engine', // Set brand to "engine" as requested
    sourceUrl: sourceUrl,
...
async function fetchEngineProducts(page = 1, limit = 250): Promise<{ products: ShopifyProduct[]; hasNext: boolean }> {
  try {
    const url = `https://engine.com.pk/products.json?page=${page}&limit=${limit}`;
    console.log(`[Engine Scraper] Fetching page ${page} from ${url}`);
    ...
    console.error(`[Engine Scraper] Error fetching page ${page}:`, error.message);
    throw error;
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[Engine Scraper] Starting to scrape products with pagination...');
    ...
    while (hasNext) {
      try {
        const { products, hasNext: hasMore } = await fetchEngineProducts(page, limit);
        totalFetched += products.length;

        console.log(`[Engine Scraper] Page ${page}: Fetched ${products.length} products (Total fetched: ${totalFetched})`);
        ...
        console.error(`[Engine Scraper] Error on page ${page}:`, error.message);
        // Continue to next page even if one fails
        hasNext = false;
      }
    }

    console.log(`[Engine Scraper] Completed. Total fetched: ${totalFetched}, Winter products: ${totalFiltered}, Filtered out: ${filteredOutCount}`);

    // Save all products to JSON file
    const allProductsJsonData = {
      scrapedAt: new Date().toISOString(),
      source: 'https://engine.com.pk/products.json',
      brand: 'engine',
    ...
    const allProductsFilePath = path.join(scrapedDir, 'engine-all-products.json');
    await fs.writeFile(allProductsFilePath, JSON.stringify(allProductsJsonData, null, 2), 'utf-8');
    console.log(`[Engine Scraper] Saved ${allScrapedProducts.length} products to ${allProductsFilePath}`);

    // Save winter products JSON file
    const winterProductsFilePath = path.join(scrapedDir, 'engine-winter-products.json');
    await fs.writeFile(winterProductsFilePath, JSON.stringify(winterProductsJsonData, null, 2), 'utf-8');
    console.log(`[Engine Scraper] Saved ${winterProducts.length} winter products to ${winterProductsFilePath}`);
    ...
    console.error('[Engine Scraper] Error:', error);
    return NextResponse.json(
