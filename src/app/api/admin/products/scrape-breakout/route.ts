...
function isWinterProduct(product: ShopifyProduct): boolean {
  ...
  const winterKeywords = [
    'winter',
    'win',
    '25-win', // Common tag pattern seen in Breakout products
    'warm',
    'wool',
    'fleece',
    'quilted',
...
function convertShopifyToProduct(shopifyProduct: ShopifyProduct): any {
  ...
  let images = shopifyProduct.images?.map(img => {
    ...
    if (img.src.startsWith('/')) {
      return `https://breakout.com.pk${img.src}`;
    }
    return img.src;
  }) || [];
  ...
  return {
    ...
    images: images,
    category: category,
    brand: 'breakout', // Set brand to "breakout" as requested
    sourceUrl: sourceUrl,
...
async function fetchBreakoutProducts(page = 1, limit = 250): Promise<{ products: ShopifyProduct[]; hasNext: boolean }> {
  try {
    const url = `https://breakout.com.pk/products.json?page=${page}&limit=${limit}`;
    console.log(`[Breakout Scraper] Fetching page ${page} from ${url}`);
    ...
    console.error(`[Breakout Scraper] Error fetching page ${page}:`, error.message);
    throw error;
  }
}
  ...
  try {
    ...
    while (hasNext) {
      try {
        ...
        console.error(`[Breakout Scraper] Error on page ${page}:`, error.message);
    ...
    const jsonData = {
      ...
      source: 'https://breakout.com.pk/products.json',
      brand: 'breakout',
